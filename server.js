...
// Book.find({rating : {$lt : 5}}).sort({author : -1}).exec(function(error, books){
//   console.log(books)
// });

//3. Find all the Fiction books, skip the first 2, and display only 3 of them 

// Book.find({$or : [{genres : "Fiction"},{genres : "fiction"}]}).skip(2).limit(2).exec( function(err, books){
//   console.log(books)
// })
...
// Person.find({$and : [{height : {$gt : 180}}, {salary : {$gt : 3000}}]},function(err, people){
//     console.log(people)
// })

//2. Find all the people who are tall (>180) OR rich (>30000)

// Person.find({$or : [{height : {$gt : 180}}, {salary : {$gt : 3000}}]}, function(err, people){
//   console.log(people)
// })
